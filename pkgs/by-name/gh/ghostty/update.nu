...
# Regenerate Ghostty.metallib. Apple's Metal compiler isn't available under
# Nix, so this only runs on macOS where xcrun is present.
if $nu.os-info.name == "macos" {
  print "Regenerating Ghostty.metallib"
  ^python3 ./pkgs/by-name/gh/ghostty/scripts/build-metallib.py
} else {
  print "Skipping Ghostty.metallib regen (requires macOS + Xcode)"
}
